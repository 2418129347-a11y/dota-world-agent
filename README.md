...
- 把顶级战队/选手的转会、退役、离队、解散和重组列为 T1 情报，优先级仅次于中国区消息。
- 可收录达到分类热度门槛的转会期社区传闻，并区分“已官宣”“多源待官宣”“媒体线索待官宣”和“社区高热未官宣”。
- 邮件内置赛程卡片：北京时间、对阵双方、阶段、BoX 与是否淘汰局；具体对阵未公布时明确显示待核验，不猜测。
- 排除笼统的赛程/结果索引、静态战队数据页，以及不属于 Tier 1 的普通全球比赛。
...
4. 默认工作流使用 15 分钟心跳覆盖 GitHub 可能出现的排队延迟：06:30–07:59 到达的任务会在云端等待至 08:00，08:00–09:29 到达的任务立即补发，09:30 后拒绝发送；成功发送后，当天其余触发自动跳过。
...
- GitHub Actions 的定时触发不提供严格准点保证；项目让 06:30–07:59 的早班心跳等待至 08:00，并为延迟任务保留至 09:29 的补发窗口。若 06:30–09:29 之间完全没有运行，当天仍可能漏发，但不会在 11 点发送。
- 本项目不提供投注、投资或博彩建议。
