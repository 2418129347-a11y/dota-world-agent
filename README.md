...
4. 默认工作流在北京时间 07:43 主跑（等待至 08:00），并在 08:17、09:17 安排两次安全补跑；09:30 后拒绝发送，成功发送后当天其余触发自动跳过。QQ SMTP 若在连接或登录阶段临时断开，会自动重试最多三次。
...
- GitHub Actions 的定时触发不提供严格准点保证；项目使用北京时间 07:43、08:17、09:17 三次低拥塞心跳，并保留至 09:29 的补发窗口。若三次触发均被平台漏掉，当天仍可能漏发，但不会在 11 点发送。
- 本项目不提供投注、投资或博彩建议。
